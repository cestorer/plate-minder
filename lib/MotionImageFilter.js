	...
	async next(rawImage) {
		...
		return await this.enqueue(rawImage, async (priorRawImage) => {
			try {
				...
				if(rawImage.buffer.length === 0) return;
				...
				//dilate the image
				cv.dilate(dst, dst, cv.Mat.ones(5, 5, cv.CV_8U), new cv.Point(-1, -1), 1, cv.BORDER_CONSTANT);
				
				//find the contours
				const contours = new cv.MatVector();
				cv.findContours(dst, contours, new cv.Mat(), cv.RETR_CCOMP, cv.CHAIN_APPROX_SIMPLE);
				
				//map the contours into bounding boxes
				const rects = Array.from({length: contours.size()}, (_, i) => cv.boundingRect(contours.get(i)));
      			contours.delete();
				...
				if(rects.length === 0)
					...
					await rawImage.crop(ROI.fromObject({
